...
export default function HelpPage() {
  ...
  const { user, profile: userProfile, loading: authLoading } = useSupabaseUser();
  ...
  const [liveChat, setLiveChat] = useState<ChatRequest | null>(null);
  ...
  useEffect(() => {
    const fetchActiveChat = async () => {
        ...
        if(user) {
            const { data } = await supabase.from('chat_requests').select('*').eq('user_id', user.id).in('status', ['pending', 'active']).order('created_at', {ascending: false}).limit(1);
            if(data && data.length > 0) activeChatId = data[0].id;
        } else {
             const savedGuestId = sessionStorage.getItem(GUEST_CHAT_ID_KEY);
             if (savedGuestId) {
                const { data } = await supabase.from('chat_requests').select('*').eq('id', savedGuestId).single();
                if (data && ['pending', 'active'].includes(data.status)) {
        ...
        if (activeChatId) {
             const { data } = await supabase.from('chat_requests').select('*').eq('id', activeChatId).single();
             setLiveChat(data);
             if (!channelRef.current) {
                const channel = supabase.channel(`chat_${activeChatId}`)
                    .on<ChatRequest>('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'chat_requests', filter: `id=eq.${activeChatId}` }, (payload) => {
                         setLiveChat(payload.new);
                    }).subscribe();
  ...
  }, [user, supabase, guestChatId]);


  const prevMessagesCount = useRef(liveChat?.chat_history?.length ?? 0);
  ...
  useEffect(() => {
    if (liveChat?.status === 'pending' && liveChat?.created_at) {
        const createdAt = new Date(liveChat.created_at);
  ...
  useEffect(() => {
    if (user && userProfile?.numeric_id) {
      setUid(userProfile.numeric_id);
    }
  }, [user, userProfile]);
  ...
  useEffect(() => {
    ...
    if (liveChat?.chat_history && liveChat.chat_history.length > prevMessagesCount.current) {
        const lastMessage = liveChat.chat_history[liveChat.chat_history.length - 1];
        if (lastMessage && !lastMessage.isUser) {
            playReceiveSound();
        }
    }
    
    if (liveChat?.chat_history) {
        prevMessagesCount.current = liveChat.chat_history.length;
    }
  }, [liveChat?.chat_history]);
  ...
  const handleStartChat = async () => {
    ...
    if (!result.success || !result.chatId) {
      toast({
        ...
        const { data } = await supabase.from('chat_requests').select('*').eq('id', result.chatId).single();
        setLiveChat(data);
    ...
    const handleSendMessage = async () => {
        if ((!currentMessage.trim() && !attachment) || !liveChat || liveChat.status !== 'active') return;
        ...
        const newHistory = [...(liveChat.chat_history || []), messagePayload];

        try {
            await supabase.from('chat_requests').update({ chat_history: newHistory }).eq('id', liveChat.id);
            setCurrentMessage('');
            setAttachment(null);
        } catch (error: any) {
    ...
    const handleCloseChat = async () => {
        if (liveChat) {
            try {
                await supabase.from('chat_requests').update({ status: 'closed' }).eq('id', liveChat.id);
            } catch (error: any) {
  ...
  if (liveChat) {
    const inputDisabled = liveChat.status !== 'active' || isSending;
    const isWaitingForAgent = liveChat.status === 'pending';
    const displayedMessages = liveChat.chat_history || [];
      ...
      <div className="flex flex-col h-screen bg-secondary">
        <header className="grid grid-cols-3 items-center p-3 bg-white sticky top-0 z-10 border-b shadow-sm">
            ...
            <div className="flex flex-col items-center text-center">
                <h1 className="text-lg font-bold">{liveChat.status === 'active' ? "JONNY" : "Support"}</h1>
